...
- Pre-flight doctor (predicts failure modes before a run; writes nothing): `arch-lw doctor <file>` → `--share` (default, redacts layer names to `L01..Ln`; `--full` for real names), `--json`, `--geometry/--no-geometry` (pass 2 counts). Exit 0 clear / 1 may-hit / 2 will-hit. `--export-geometry L04,L11|flagged [--export-out f.json]` writes redacted anchor geometry (per-layer translate to origin, never scaled, so gaps survive) for checkpointing a stubborn layer into a fixture without the drawing leaving the machine.
...
- `doctor`'s color findings count *usable* colors (`doctor.usable_color_count`), not `len(stroke_colors)`: `inspect._color_key` keys by colorspace, so a CMYK export has stroke colors that `classify.auto_by_role` discards. Counting raw keys reports a healthy ladder for a file that cannot be weighted.
- `doctor --share` output is paste-safe by invariant: every non-numeric token is a literal from arch-lw's own rule tables or the report template. `tests/test_doctor.py` enforces it differentially on both secret channels (two drawings whose layer names share a shape but no letters, and two files whose PDF producer/creator metadata differ only in identifying text, must each render identically). Adding prose to `doctor.render_text` is fine; emitting anything derived from a layer name, a file path or the producer/creator strings is not. Note `diagnose` prints real layer short names and is NOT paste-safe.
