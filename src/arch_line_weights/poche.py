        ...
        for b in segments[idx + 1 :]:
            ...
            key = tuple(
                sorted(
                    (
                        (
                            round(float(bridge_points[0][0]), 4),
                            round(float(bridge_points[0][1]), 4),
                        ),
                        (
                            round(float(bridge_points[1][0]), 4),
                            round(float(bridge_points[1][1]), 4),
                        ),
                    )
                )
            )
...
(function () {
    ...
    function shouldDump(name) {
        ...
        if (n.indexOf("::VISIBLE::CURVES::") === -1 &&
            n.indexOf("::VISIBLE::TANGENTS::") === -1) return false;
