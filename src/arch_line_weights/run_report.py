...
def _review_reasons(
    ...
    *,
    ...
    reasons: list[str] = []
    if status == "skipped":
        reasons.append("skipped by override")
    if status in {"low_confidence", "failed", "missing_payload"}:
        reasons.append(status.replace("_", " "))
    if fill.strategy in _REVIEW_STRATEGIES:
        reasons.append(f"strategy {fill.strategy} is review-only")
    if 0 < fill.confidence < 0.85:
        reasons.append(f"confidence {fill.confidence:.2f} below 0.85")
    if _requires_visual_acceptance(fill):
        reasons.append("inferred concrete/foundation fill requires W5/W7 visual acceptance")
    if missing_payload:
        reasons.append("payload layer could not be located for injection")
    reasons.extend(extra_review_reasons or [])
    ...
    *,
    ...
    for fill in poche_report.fills:
        ...
        if any(_meaningful_rejection(candidate) for candidate in layer_candidates):
            extra_reasons.append("one or more Make2D completion candidates were rejected")
        ...
        if limitation is not None:
            ...
            extra_reasons.append("foundation/concrete coverage is launch-blocking")
...
def build_poche_report(
    *,
    ...
    limitations.extend(_known_limitations(known_limitations))
    polygons_injected = sum(len(polys) for polys in poche_report.polygons.values())
    summary: dict[str, Any] = {
        ...
        "polygons_injected": polygons_injected,
        "polygons_filled": polygons_injected,
        "polygons_diagnostic_only": diagnostic_polygons,
        "bytes_injected": 0,
        "limitations_count": len(limitations),
        "no_go_limitations": _no_go_limitations(limitations),
