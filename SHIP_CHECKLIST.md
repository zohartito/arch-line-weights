...
- [x] `arch-lw apply --preset studio` is supported.
- [x] `arch-lw apply-saas --preset studio` is supported.
- [x] README explains that basic `apply` is fast stroke-weight output.
- [x] README explains that `apply-saas --architectural --poche --preset studio --source rhino` is a local review path for Rhino 8 -> Illustrator `.ai` -> Illustrator/Acrobat inspection.
...
- [x] Studio public print texture/hatch convention is 0.13 mm; 0.08 pt remains screen-review only.
...
- [ ] Real sample fixture: large real `.ai` drawings remain excluded from git. Decide whether to add a small anonymized `.ai`, use Git LFS/private release assets, or keep only the tiny PDF smoke fixture.
